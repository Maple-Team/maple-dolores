  ...
  <div class="p-4 bg-white">
    ...
    <template v-else>
      ...
      <a-timeline>
        <a-timeline-item
          color="green"
          v-if="notTodayRecords?.length"
        >
          <p class="text-green-500">
            {{ notTodayRecords[0].date }}
          </p>
          <p
            ...
            class="mb-0"
          >
            <a-tag color="rgb(34 197 94)">#{{ capitalize(record.type) }}</a-tag
...
const notTodayRecords = computed(() => data.value?.records.filter(({ date }) => date !== currentDate))
