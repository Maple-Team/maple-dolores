<template>
  <a-form :label-col="labelCol" :wrapper-col="wrapperCol">
    ...
    <a-tabs v-model:activeKey="activeKey">
      <a-tab-pane key="input" tab="Textarea">
        ...
        <a-form-item label="Chapter No." v-bind="validateInfos.chapterNo">
          <a-input-number v-model:value="modelRef.chapterNo" :min="0" :step="1"></a-input-number>
        </a-form-item>
        <a-form-item label="Chapter Content" v-bind="validateInfos.chapterContent">
          <a-textarea v-model:value="modelRef.chapterContent" allowClear></a-textarea>
        </a-form-item>
      ...
      <a-tab-pane key="upload" tab="Upload">
        <a-upload multiple :file-list="fileList" :before-upload="beforeUpload" @remove="handleRemove">
          <a-button>
    ...
    <a-form-item label="labels">
      <a-select v-model:value="modelRef.labels" mode="tags" style="width: 100%" :token-separators="[',']"
        placeholder="选择或输入标签">
        <a-select-option :value="label.id" v-for="label in labels" :key="label.id">{{label.name}}</a-select-option>
      </a-select>
    </a-form-item>
